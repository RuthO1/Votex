...
const upload = (file) => {
  ...
  export const addCandidate = async (name, age, image, description, candidateAddress, address, votingContract ) => {
 ...
 export const getCandidate = async ( votingContract ) => {
    try {
        const candidates = [];
        const candidatesLength = await votingContract.methods.getCandidateLength().call();
        for (let i = 0; i < Number(candidatesLength); i++) {
            const candidate = new Promise(async (resolve) => {
                ...
                resolve({
                    ...
                    name: meta.name,
                    age: meta.age,
                    candidateAddress: meta.candidateAddress,
                    image: meta.image,
...
export const givetVotingRight = async ( votingContract, _address, address ) => {
    try {
        await votingContract.methods.giveVoterRight(_address).send({ from: address });
